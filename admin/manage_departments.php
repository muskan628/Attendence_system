...
if ($res->num_rows == 0) {
  $res2 = $conn->query("SHOW TABLES LIKE 'department'");
  if ($res2->num_rows > 0) {
    $deptTable = 'department';
  }
}
// If column name varies (name vs DEPARTMENT_NAME), ideally check that too, 
// but existing code seems to assume 'name'. 
// Let's stick to table name fix first.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['department_name'])) {
  ...
  if ($name !== '') {
    // Generate Custom ID (D-XXXX) - uses table name dynamically if passed, but id_helper might need update or we pass $deptTable
    $newId = generateCustomId($conn, 'D', $deptTable, 'id', 4);

    $stmt = $conn->prepare("INSERT INTO $deptTable (id, name) VALUES (?, ?)");
    if (!$stmt) {
...
if (isset($_GET['delete'])) {
  $id = $_GET['delete'];
  $stmt = $conn->prepare("DELETE FROM $deptTable WHERE id = ?");
...
/* ---------- FETCH LIST ---------- */
$result = $conn->query("SELECT * FROM $deptTable ORDER BY id ASC");
